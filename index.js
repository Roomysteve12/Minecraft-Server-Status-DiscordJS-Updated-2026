...
client.on('clientReady', async () => {
  ...
  async () => {
    ...
    if(config.software === 'bedrock') {
      ...
      let { motd, players, online } = await fetch(`https://api.mcstatus.io/v2/status/bedrock/${config.ip}:${config.port}`).then(response => response.json())
      ...
      if(online === true) {
        client.user.setPresence({ activities: [{ name: `Bedrock Server | Status: Online | Players: ${players.online}/${players.max}` }]  })     
      }
    }
    ...
    if(config.software === 'java') {
      ...
      let {motd, players, online } = await fetch(`https://api.mcstatus.io/v2/status/java/${config.ip}:${config.port}`).then(response => response.json())
      ...
      if(online === true) {    
        ...
        client.user.setPresence({ activities: [{ name: `Java Server | Status: Online | Players: ${players.online}/${players.max}` }]  })
      }
    }
